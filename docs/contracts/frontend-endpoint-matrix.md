...
| **Attach a document** | `POST /api/v1/me/kyc/documents` | bearer | — | own-record | multipart `file`, `type` | the document slot | **F28** — nothing attached a file to a KYC case, so a claim could be submitted without the document that settles it. `type` is a closed set (`identity-document`, `proof-of-address`) and is the **slot key**, so no file identifier is ever named by a client. **No selfie, no liveness, no biometric** — a facial image is `Sensitive` and is not revocable. Classification is set server-side to `Personal`; an applicant cannot under-classify their own ID. Accepted **only while the case is editable** (`draft`, `needs-more-information`): a document arriving after submission changes what a reviewer already saw. Throttled `uploads`. | `implemented` |
| **My documents** | `GET /api/v1/me/kyc/documents` | bearer | — | own-record | — | one row per type | Every type is listed whether attached or not, so an applicant can see they have sent nothing — the state most easily misread as sent. **No verification status and no reviewer note**: that is deliberation, and an applicant is shown the decision on their case, not the working. | `implemented` |
| **Open my document** | `POST /api/v1/me/kyc/documents/{type}/access` | bearer | — | own-record | — | short-lived `handle` | Slot resolved from the caller's own case, so there is no version id to substitute. `for_sharing` is never set — an outward disclosure is the office's decision to make and record. | `implemented` |
| **Open a case document** | `POST /api/v1/admin/kyc-cases/{case}/documents/{type}/access` | bearer | `kyc.review` | — | — | short-lived `handle` | The reviewer half of F28. Without it the applicant's documents are a write nobody reads, which is worse than refusing them: the resident believes the office holds their ID. Scoped to the case in the path; the read is audited. | `implemented` |
...
| Family list | `GET /api/v1/admin/families` | bearer | `resident.view` | role scope, **through the household** | `?q=&status=&unhoused=&page=&per_page=` | paginated summaries + derived `member_count` | a family has no barangay of its own, so scope is applied through `households`; a family with no household is invisible to a barangay-scoped caller rather than falling through to everyone | `implemented` |
| Family detail | `GET /api/v1/admin/families/{family}` | bearer | `resident.view` | role scope, through the household | — | family + members + relationships **inside** it | **audited read** — a relationship reaching outside the family is withheld, because a link is itself information about somebody the caller did not ask for | `implemented` |
| Families of a resident | `GET /api/v1/admin/residents/{resident}/families` | bearer | `resident.view` | role scope | `?page=&per_page=` | paginated family summaries | a collection whose invariant currently holds it to one member — see G-24 | `implemented` |
| Kinship history | `GET /api/v1/admin/residents/{resident}/kinship-history` | bearer | `resident.view` | role scope | `?page=&per_page=` | paginated membership and relationship events, newest first | assembled from the effective-dated tables that already exist, never a second event log; carries dates and identifiers, never rendered sentences | `implemented` |
| My work queue | `GET /api/v1/admin/work/mine` | bearer | `task.view` | **own subject id, from the token** | `?page=&per_page=` | paginated items + `meta.as_of` | there is no `?assignee=`: "mine" that can be pointed at a colleague is not mine. Rows carry a type and an opaque subject id and **no subject summary** (ADR 0024 §2) — a preview line would disclose who each task is about to everyone who can see the queue | `implemented` |
| Team work queue | `GET /api/v1/admin/work/team` | bearer | `staff.view` | office-wide | `?page=&per_page=` | groups by carrier + `meta.unassigned_count`, `meta.as_of` | **`staff.view`, not `task.view`** — reading a colleague's caseload is supervision, not something that comes free with having a queue. Unassigned work is its own group and its own figure, never an omission | `implemented` |
| Office alerts | `GET /api/v1/admin/work/alerts` | bearer | `task.view` | office-wide | `?page=&per_page=` | derived conditions + `meta.as_of` | nothing is stored and there is no dismiss: an alert you can clear without fixing anything trains an office to ignore the real ones. Each states the rule it applied and what it counted | `implemented` |
| Beneficiary registry | `GET /api/v1/admin/beneficiaries` | bearer | `program.view` | role scope | `?q=&barangay_id=&page=&per_page=` | paginated projections keyed on `resident_id` | **a projection, never an entity** — no beneficiary table, no beneficiary id, no stored standing. `program.view` rather than `resident.view`: a resident row says a person exists, a beneficiary row says what this office has done for them | `implemented` |
| Beneficiary detail | `GET /api/v1/admin/beneficiaries/{resident}` | bearer | `program.view` | role scope | — | one projection | the four standings are derived at read time and are not exclusive; in-kind releases are counted beside the money, never summed into it as zero | `implemented` |
| Duplicate findings | `GET /api/v1/admin/residents/{resident}/duplicate-findings` | bearer | `resident.merge` | role scope | `?page=&per_page=` | paginated findings, newest first | the rule that matched, the resemblance band, the verdict and the reason — **never the field values** (`DL-73`). Behind `resident.merge`: reading who was judged the same person as whom is identity administration, not registry browsing | `implemented` |
| Report catalogue | `GET /api/v1/admin/reports` | bearer | `report.view` | role scope | `?page=&per_page=` | reports **this caller may run**, each with its question | filtered by permission, not annotated with it — a listing that names what somebody may not have tells them it exists, and `release-manifest` existing is a fact about how this office pays people | `implemented` |
| Run a report | `POST /api/v1/admin/reports/{report}/run` | bearer | the report's own permission | role scope | filters the definition declares | rows + `meta.suppression` | **aggregate only** — a report that names people is an export, because the retention window and audit entry that come with one are the point. `assigned_to` is refused here and permitted on the dashboard: a report is the artefact pasted into a meeting pack, and a per-worker report is a league table however it was produced. Cells below 5 are withheld, never rounded and never zeroed | `implemented` |
| Requirement templates | `GET /api/v1/admin/programs/{program}/requirement-templates` | bearer | `program.manage` | — | `?page=&per_page=` | every published version, newest first, each requirement flagged current or not | the superseded wording **is** the evidence: a request approved in March under version 1 must stay explicable in December. `program.manage` rather than `program.view` — the current requirements are already on the programme detail; the history of what the office used to demand is administration of the catalogue | `implemented` |
| Programme utilisation | `GET /api/v1/admin/programs/utilization` | bearer | `program.view` | role scope | `?from=&to=&barangay_id=` | rows + `meta.suppression` | runs the reporting module's own metric rather than counting again — a second implementation of "how much has gone out" is a second answer. Small cells withheld unchanged | `implemented` |
| One programme's utilisation | `GET /api/v1/admin/programs/{program}/utilization` | bearer | `program.view` | role scope | as above, scoped to the programme | rows + `meta.suppression` | "1 recipient" plus a programme name is an identification, so the same suppression applies | `implemented` |
| Data classification register | `GET /api/v1/admin/privacy/classifications` | bearer | `privacy.manage` | — | — | categories, each with its RA 10173 classification, what it holds and its retention period | **reference data about nobody**, built from the same category list as the retention schedule — one set of record kinds, two facts about each, and two lists are how they come to disagree. A category with a period and no classification is reported, not skipped: a gap in a privacy register is the finding | `implemented` |
| Intake advisory | `GET /api/v1/admin/assistance-requests/{case}/advisory` | bearer | `request.view` | role scope | — | signals, `records_read`, `windows` | **advises and never decides** — no score, no total, no `eligible`, no recommendation, and the shape has nowhere to put one. Two tones and neither blocks. Moved here from the console in TAB 07: a rule that shapes how an applicant is treated, living where the client can change or skip it, is not a rule the office can say it applied | `implemented` |
| Post history | `GET /api/v1/admin/newsfeed/{post}/history` | bearer | `newsfeed.manage` | — | `?page=&per_page=` | the post's own lifecycle, newest first | read from the post's dated columns, **not** the audit trail: `audit.view` is withheld from all but the DPO, so a manager reading their own post would otherwise need the permission that opens every approval in the office | `implemented` |
| Event history | `GET /api/v1/admin/events/{event}/history` | bearer | `event.manage` | — | `?page=&per_page=` | the event's own lifecycle, with the cancellation reason | same reasoning as the post history. People arranged their day around the event; "cancelled" with no reason is the version that wastes the trip | `implemented` |
| Reconciliation | `GET /api/v1/admin/releases/reconciliation` | bearer | `request.release` | role scope | `?from=&to=` | totals by status, by programme, and overall | the disbursing officer's ledger view, to tie against a cash count. **Not a report and not suppressed** — a withheld row makes a balance impossible, and the narrower permission is what pays for that. Carries no names. Goods are counted beside the money, never summed in as zero | `implemented` |
| Distribution runs | `GET /api/v1/admin/release-batches` | bearer | `request.view` | role scope | `?scheduled_for=&page=&per_page=` | paginated batches with derived counts | **counts, never a status of its own** (`DL-90`): "38 of 41 released, 2 deferred" names the problem where "partially complete" hides the two people still waiting | `implemented` |
| One distribution run | `GET /api/v1/admin/release-batches/{batch}` | bearer | `request.view` | role scope | — | one batch, same derived counts | ″ | `implemented` |
...
| Record a sector | `POST /api/v1/admin/residents/{resident}/sectors` | bearer | `resident.manage` | role scope | `{sector,reason}` | `201` `{resident_id,sector}` | idempotent — recording twice leaves one row, because a duplicate would double every sectoral count the LGU reports. A **safeguarding sector (`vawc-survivor`, `cicl`) additionally requires `request.view-sensitive`**, so `resident.manage` alone cannot flag somebody | `implemented` |
| End a sector | `DELETE /api/v1/admin/residents/{resident}/sectors/{sector}` | bearer | `resident.manage` | role scope | `{reason}` | `{resident_id,sector,ended}` | a recorded act, not an erasure — a solo parent whose child turns eighteen stops being one. Gated identically: removing a safeguarding flag is as consequential as adding it | `implemented` |
...
| Screen / caller | Endpoint | Auth | Permission | Scope | Request | Response | Sensitivity | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Case queue | `GET /api/v1/admin/assistance-requests` | bearer | `request.view` | role scope | `?status=&type=&open_only=&assigned_to=me&unassigned=&page=&per_page=` | paginated summaries | **protective cases excluded from rows and from the total** without `request.view-sensitive`; `assigned-cases` scope narrows to the caller's own | `implemented` |
| Case file | `GET /api/v1/admin/assistance-requests/{case}` | bearer | `request.view` | role scope | — | operational fields + `available_transitions[]` | **audited read**; deliberately carries **no** vulnerability snapshot (ADR 0016 §4) | `implemented` |
| Open a case | `POST /api/v1/admin/assistance-requests` | bearer | `request.create` | role scope | `{resident_id,type,household_id?,program_id?}` | `201` case | a `protective` type additionally requires `request.view-sensitive` — opening one is itself a protection decision | `implemented` |
| Lifecycle transition | `POST /api/v1/admin/assistance-requests/{case}/transitions` | bearer | **resolved from the target state** | role scope | `{to,reason?,applicant_message?}` | case | legality checked **before** permission (`409` beats `403`), or the error maps the authorization table; `reason` mandatory for rejected/cancelled/returned/completed/expired | `implemented` |
| Priority | `POST /api/v1/admin/assistance-requests/{case}/priority` | bearer | `request.assign` | role scope | `{priority,reason?}` | case | `urgent` requires a reason; **never derived from a vulnerability score** | `implemented` |
| Assign | `POST /api/v1/admin/assistance-requests/{case}/assignment` | bearer | `request.assign` | role scope | `{assignee_subject_id,team?}` | case | idempotent; a closed case cannot be assigned; routing is assignment, never state | `implemented` |
| Return to queue | `DELETE /api/v1/admin/assistance-requests/{case}/assignment` | bearer | `request.assign` | role scope | `{reason}` | case | an unassigned open case is the backlog, a first-class state | `implemented` |
| Archive | `POST /api/v1/admin/assistance-requests/{case}/archive` | bearer | `request.close` | role scope | — | case | only a terminal case; a flag, not a status | `implemented` |
| History | `GET /api/v1/admin/assistance-requests/{case}/history` | bearer | `request.view` | role scope | — | transitions + assignments + timeline | audited read; internal `reason` visible here and nowhere a citizen can reach | `implemented` |
...
| Intake + assessment | `GET /api/v1/admin/assistance-requests/{case}/assessment` | bearer | `request.view` | role scope | — | intake detail + current assessment | the applicant's narrative is staff-facing and behind the audited case file | `implemented` |
| Open assessment | `POST /api/v1/admin/assistance-requests/{case}/assessment` | bearer | `request.assess` | role scope | `{template_code}` | `201` assessment | **pins `template_version` at open**; idempotent — a second call returns the one in progress | `implemented` |
| Record answers | `PATCH /api/v1/admin/assistance-requests/{case}/assessment` | bearer | `request.assess` | role scope | `{answers:{code:value}}` | assessment | choice answers validated against the pinned template; a **completed** assessment cannot be edited | `implemented` |
| Sign findings | `POST /api/v1/admin/assistance-requests/{case}/assessment/complete` | bearer | `request.assess` | role scope | `{recommendation,reason?,findings?}` | assessment + `suggested_next_status` | **moves nothing** — see below; required answers must be present; `recommend-deny` requires a reason | `implemented` |
| Prior cases | `GET /api/v1/admin/assistance-requests/{case}/prior-cases` | bearer | `request.assess` | role scope | — | identity, category, status, dates | **no narratives, no assessments, no amounts** — knowing somebody has come three times is a different question from reading what they said | `implemented` |
...
| Screen / caller | Endpoint | Auth | Permission | Scope | Request | Response | Sensitivity | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Run a check | `POST /api/v1/admin/assistance-requests/{case}/eligibility-checks` | bearer | `request.assess` | role scope | `{program_id}` | `201` check + per-criterion results | **advisory** (`is_advisory: true` in the payload); pins `guidance_version`; **moves nothing** | `implemented` |
| Check history | `GET /api/v1/admin/assistance-requests/{case}/eligibility-checks` | bearer | `request.view` | role scope | — | every check ever run | append-only; the audit record behind a decision | `implemented` |
...
| Screen / caller | Endpoint | Auth | Permission | Scope | Request | Response | Sensitivity | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Requirement checklist | `GET /api/v1/admin/assistance-requests/{case}/requirements` | bearer | `request.view` | case barangay | — | slots, satisfaction, outstanding count, upload limits | `accepts` publishes the same limits the server enforces, so a client's copy cannot drift | `implemented` |
| Attach a programme's template | `POST /api/v1/admin/assistance-requests/{case}/requirements` | bearer | `document.manage` | case barangay | `{program_id}` | `201` requirements | **copied, not referenced** — a case stays explicable against the list in force when it opened | `implemented` |
| Record a document | `POST /api/v1/admin/assistance-requests/{case}/requirements/{requirement}/documents` | bearer | `document.manage` | case barangay | multipart `{source,file?,document_number?,issued_on?,expires_on?,expiry_unknown?,replaces_because?}` | `201` version | type read from the file's **own bytes**; `415` on a mismatch, `413` over 10 MiB; a replacement **requires a reason** (`422`) | `implemented` |
| Version history | `GET /api/v1/admin/assistance-requests/{case}/requirements/{requirement}/documents` | bearer | `request.view` | case barangay | — | every version ever presented | superseded versions included, with reason and their own file — that is the point | `implemented` |
| Accept / refuse | `POST /api/v1/admin/assistance-requests/{case}/requirements/{requirement}/verification` | bearer | `document.verify` | case barangay | `{status,note?}` | version | **not `document.manage`** — the clerk who took the paper is not the one who judged it; a rejection **must** say why (`422`); a superseded version is `409` | `implemented` |
| Rule a conditional requirement | `POST /api/v1/admin/assistance-requests/{case}/requirements/{requirement}/applicability` | bearer | `document.verify` | case barangay | `{applicability,reason}` | requirement | reason **mandatory in both directions** — ruling a document out is the step that can waive a safeguard | `implemented` |
| Open a document | `POST /api/v1/admin/assistance-requests/{case}/requirements/{requirement}/documents/{version}/access` | bearer | `request.view` (+`document.view-sensitive`, +`document.share`) | case barangay | `{for_sharing?}` | single-use handle + expiry | version must sit in **this** requirement's slot, else `404`; sharing additionally needs a permission **nobody holds yet** | `implemented` |
...
| Screen / caller | Endpoint | Auth | Permission | Scope | Request | Response | Sensitivity | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Requests on a case | `GET /api/v1/admin/assistance-requests/{case}/document-requests` | bearer | `request.view` | case barangay | — | open first, then most recent | `is_applicant_overdue` — named for its subject, because a case task is late when *staff* are | `implemented` |
| Ask for a document | `POST /api/v1/admin/assistance-requests/{case}/requirements/{requirement}/document-requests` | bearer | `document.manage` | case barangay | `{channel,message,needed_by?}` | `201` request | `message` **mandatory** — a record that something was asked for without saying what looks like follow-up the office cannot show; a past `needed_by` is `422` | `implemented` |
| No longer needed | `POST /api/v1/admin/assistance-requests/{case}/document-requests/{documentRequest}/withdraw` | bearer | `document.manage` | case barangay | `{reason}` | request | withdrawn, never deleted — an applicant who was chasing a document deserves the record that released them | `implemented` |
...
| Screen / caller | Endpoint | Auth | Permission | Scope | Request | Response | Sensitivity | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Notes | `GET /api/v1/admin/assistance-requests/{case}/notes` | bearer | `request.view` | case barangay | — | notes + `withheld_count` + `has_safeguarding_concern` | a protected note's **existence, author, sensitivity and time are disclosed**; only `body` is null | `implemented` |
| Add a note | `POST /api/v1/admin/assistance-requests/{case}/notes` | bearer | `request.view` (+`case-note.view-protected` for the protected tier) | case barangay | `{body,sensitivity?}` | `201` note | writing into the protected tier needs the same clearance as reading it — otherwise a note can be put beyond *review* rather than beyond disclosure | `implemented` |
| Withdraw | `POST /api/v1/admin/assistance-requests/{case}/notes/{note}/withdrawal` | bearer | `request.view` | case barangay | `{reason}` | note | withdrawn, never deleted, and **only by its author** — a record of what one worker believed is not another's to retract | `implemented` |
...
| Prepare | `POST /api/v1/admin/assistance-requests/{case}/releases` | bearer | `request.schedule` | case barangay | `{kind,amount_centavos?,in_kind_description?,release_mode,program_id?,funding_source?,scheduled_for?}` | `201` release | **only against an approved case** (`409`); cash needs an amount, in-kind must **not** have one; `sequence` assigned inside a lock | `implemented` |
...
| **Report** | `POST /api/v1/newsfeed-comments/{comment}/reports` | bearer | — | any resident but the author | `{reason}` | `{status}` | **F26** — required by both app stores for user-generated content. `reason` is a **closed vocabulary** (`abusive`, `harassment`, `false-information`, `spam`, `personal-information`) with **no `other` and no free text**: a text box here is where a resident writes personal data about a neighbour into a municipal record. **Flags, never hides** — moves a visible comment to `review-needed`, and a person decides; hiding on report hands residents a veto over each other. Idempotent per person, and the **response never varies** — reported now, already reported, reported by nine others: same `202`, because any difference publishes other residents' actions. | `implemented` |
