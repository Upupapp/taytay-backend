...
| `Files` | stored objects on the private disk, the documents presented against them, their append-only version history, single-use access grants, and the **re-encoded public renditions** derived when another module publishes its content | **who may see a document** — it cannot know; the owning module authorises and then calls in (ADR 0020 §1). Also: what a document *means* to a case; any HTTP surface of its own; and **whether content is published** — it derives a public rendition when told, and refuses for anything but `public-reference` material, so a module that attached the wrong file cannot publish it by being wrong (ADR 0033 §3) | **implemented** (TAB 15, TAB 28) |
...
| Akamai Object Storage (`public-media` disk) | **separate bucket, separate credentials** for re-encoded renditions of published newsfeed and event images | `Files` (`MediaPublisher` only) | any uploaded object, of any classification — nothing is copied here, only derived (ADR 0033 §4) |
